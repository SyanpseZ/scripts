...
local Stats = game.Players.LocalPlayer:FindFirstChild("data1")
if not Stats then
...
local function Run()
    ...
    Section:NewKeybind("Attack Keybind", "KeybindInfo", Enum.KeyCode.Z, function()
        local CombatRemote = game:GetService("ReplicatedStorage"):FindFirstChild("Combat")
        if CombatRemote then
            CombatRemote:FireServer(selectedHit, 0, 1000, selectedHitArea)
        else
            warn("Combat Remote not found!")
        end
    end)
    ...
    local trainerOptions = {"Hitman", "Peekaboo", "Crouching", "Refined Orthodox", "Long Guard", "Philly Shell"}
    Section:NewDropdown("Select Trainer", "DropdownInf", trainerOptions, function(currentOption)
    ...
    Section:NewButton("Tp To Trainer", "ButtonInfo", function()
        if SelectedTrainer and SelectedTrainer:FindFirstChild("Torso") then
            humanoidrootpart.CFrame = SelectedTrainer.Torso.CFrame
        else
            warn("Trainer or Torso not found!")
        end
    end)
